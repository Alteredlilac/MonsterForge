"""
Tests for the MVP 1 FastAPI web app (ui/app.py): POST /review and
POST /review/edit, the cards library, and reopening a past event for
review. GET/POST /convert moved to tests/ui/routes/test_convert.py
once that route split into its own module (ui/routes/convert.py) --
review/library routes are still in ui/app.py itself, pending the same
treatment.
