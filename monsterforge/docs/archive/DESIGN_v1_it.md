> **Historical document (Italian, original design notes).** Kept for
> reference. The numeric algorithms here (VITA, CORPO/SPIRITO,
> ARMATURA/TALISMANO, INTERPRETAZIONE, FIATO/MAGIA) are still accurate and
> implemented as described. The LLM classification schema and the draft
> dataclass sketches are superseded by the actual implementation — see the
> current [DESIGN.md](../../../DESIGN.md) (English) for what changed and
> why. For current project state see
