"""
Write-path persistence operations for the Attack classification
pipeline: creating/reusing a raw_field, recording an LLM classification
or a human review decision as a new append-only event, activating one
as a raw_field's current result, and saving the resulting
structured_data/cards rows.
...
Two sibling modules hold what doesn't belong here: reference_lookups.py
(seeded Game/Actor row lookups -- a different question, and not
attack-specific at all) and attack_repository_queries.py (read-only
queries over the data this module saves -- serving an already-cached
card, browsing the saved-cards library, resolving history -- none of
which ever calls session.add()/session.commit()).
"""
import datetime
import hashlib
import json
